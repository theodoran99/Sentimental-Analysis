...
with st.expander('Analyze CSV'):
    ...
    if upl:
        df = pd.read_csv(upl)
        column_name_sentiment = st.selectbox('Select column for sentiment analysis:', df.columns)
        df['score'] = df[column_name_sentiment].apply(score)
        ...
        if lookup_words:
            ...
            for word in words:
                ...
                if not filtered_df.empty:
                    ...
                    plt.figure()
                    sns.scatterplot(data=filtered_df, x=filtered_df.index, y='score', hue='analysis',
                                    palette='viridis')
                    plt.xlabel('Index')
                    plt.ylabel('Sentiment Score')
                    plt.title(f'Sentiment Analysis Scatter Plot for "{word}" in column "{column_name_lookup}"')
                    st.pyplot()
                else:
                    st.write(f"The word '{word}' doesn't exist in the '{column_name_lookup}' column.")
